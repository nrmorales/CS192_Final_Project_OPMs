...
<th width="802" class="maintable" scope="col">						<?php
        ...
        if ($guest == false) {
            $add = <<<ADD
						<form name="additem" action="additem.php" onsubmit=" return add(this, 'Item')">
...
ADD;
            $query = "SELECT category_id, category FROM category_item";
...
<table width="70%" border="1" align="center" cellpadding="2" cellspacing="0" class="liststyle">
    ...
    <?php
    /*
      This section is for displaying the items in table format
     */
    $query = "SELECT name, price, stock FROM items";
    $result = mysql_query($query) or die(mysql_error());
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $output .=<<<OUTPUT
							<form name="itemtable" action="transact.php?type=Item" method="post">
								<tr>
...
OUTPUT;
            ...
            $output .=<<<OUTPUT
								...
								<td align="center"> <a href="edititems.php?name=$name"> Edit Item </td>
OUTPUT;
        }
        $output .= "</tr>";
    }
    if ($guest == true)
        $output .= "</table> <input type='submit' value='Order Item'";
    echo $output;
    ?>
