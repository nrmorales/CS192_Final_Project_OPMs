...
<script type="text/javascript">
	function ask(name){
		var toreturn = confirm("Are you sure you want to cancel\n" + name + "?");
		return toreturn;
	}
</script>
...
WRITE;
	    ...
	    <table width="1020" height="200" border="1" align="center" class="maintable">
          <tr>
            ...
            <th width="802" class="maintable" scope="col">			<table width="70%" border="1" align="center" cellpadding="0" cellspacing="0" class="liststyle">
				<tr>
				...
				<?php
					$flag = 0; //this will have a value of 1 if there is something to display in the table
					$total = 0.00;
					/*
						This is the query for temp_service table in the database and it displays the table
						for the order receipt
					*/
					$query =<<<QUERY
						SELECT temp_service.name, services.price, temp_service.quantity 
...
QUERY;
					...
					while($row = mysql_fetch_array($result)){
						...
						<tr>
							...
							<td align="center"> <a onclick="return ask('$name')" href="cancelrequest.php?tocancel=$name&option=1"> Cancel </a> </td>
OUTPUT;
					...
					/*
						This is the query for temp_item table in the database and it displays the table
						for the order receipt
					*/
					$query =<<<QUERY
						SELECT temp_item.name, items.price, temp_item.quantity 
...
QUERY;
					...
					while($row = mysql_fetch_array($result)){
						...
						<tr>
							...
							<td align="center"> $quantity </td>
							<td align="right"> $temp </td>
							<td align="center"> <a onclick="return ask('$name')" href="cancelrequest.php?tocancel=$name&option=2"> Cancel </a> </td>
OUTPUT;
...
OUTPUT;
				...
				?></th>
					...
					<?php
				/*
					Retrieve client data if client is logged on 
				*/
				if($user!='guest'){
					$query = "SELECT person_id FROM client WHERE username = '$user'";
					$result = mysql_query($query) or die(mysql_error());
					$row = mysql_fetch_array($result);
					$pid = $row['person_id'];

					$query = "SELECT * FROM person WHERE person_id = '$pid'";
					$result = mysql_query($query) or die(mysql_error());
					$row = mysql_fetch_array($result);
					...
					$client_mobile = $row['mobile_no'];
				}

				/*
					This is the form for the client to fill out
				*/
				if($flag==1){
					$add =<<<ADD
						<br>
						<form onsubmit="return validate(this)" method="post" action="doorder.php">
ADD;
						
					if($user=='guest'){
						$add .=<<<ADD
							<label for="number"> Username(optional): </label>
							<input type="text" name="puser">
							<br>
			
							<label for="number"> Password(optional): </label>
...
ADD;
					...
					$add .=<<<ADD
						...
						<input type="text" name="pname" value='$client_name'>
						<br>

						<label for="address"> Address: </label>
						<input type="text" name="paddress" value='$client_address'>
						<br>

						<label for="email"> Email Address: </label>
						<input type="text" name="pemail" value='$client_email'>
						<br>	

						<label for="number"> Mobile no: </label>
						<input type="text" name="pnumber" value='$client_mobile'>
						<br>
 
ADD;
					if($user=='guest') $add .= "<input type='submit' name='Submit' value='Register'>";
