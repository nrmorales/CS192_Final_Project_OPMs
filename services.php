...
<th width="802" class="maintable" scope="col">			<?php
        /*
          This is the form for add service
         */
        if ($guest == false) {
            $add = <<<ADD
						<form name="addservice" action="addservice.php" onsubmit=" return add(this, 'Service')">
...
ADD;
            $query = "SELECT category_id, category FROM category_service";
...
<table width="70%" border="1" align="center" cellpadding="2" cellspacing="0" class="liststyle">
    <tr>
        <th> Service name </th>
        <th> Service price </th>
        <?php
        if ($guest == true)
            echo '<th> Order Service </th>';
        else
            echo '<th> Edit Service </th>';
        ?>
    </tr>

    <?php
    /*
      This section is for displaying the services in table format
     */
    ...
    while ($row = mysql_fetch_array($result)) {
        ...
        $output .=<<<OUTPUT
							<form name="servicetable" action="transact.php?type=Service" method="post">
								<tr>
...
OUTPUT;
        ...
        if ($guest == true) {
            $output .=<<<OUTPUT
								<td align="center"> <input type="text" name="order[$name]" value="Enter number" onfocus="nullify(this)"> </td>
OUTPUT;
        } else {
            $output .=<<<OUTPUT
								<td align="center"> <a href="editservice.php?name=$name"> Edit Service </td>
OUTPUT;
        }
        $output .= "</tr>";
    }
    if ($guest == true)
        $output .= "</table> <input type='submit' value='Order Service'>";
    echo $output;
    ?>
