...
<th width="802" class="maintable" scope="col">			<?php
        ...
        $output .=<<<OUTPUT
				<form name="edititem" method="post" action="edit.php?type=item&id=$id&name2=$name" onsubmit=" return edit(this, 'Item')">
					<label> Name: </label> <input type="text" name="name" size="40" value='$name'> <br>
					<label> Info: </label> <textarea rows="5" cols="50" name="info" value='$info'>  </textarea> <br>
					<label> Price: </label> <input type="text" name="price" value='$price'> <br>
					<label> Stock: </label> <input type="text" name="stock" value='$stock'> <br>
					<label> Category: </label> <select id="category" name="category[]">
						<option value="0"> --Choose category-- </option>
OUTPUT;

        //select box for category table
        $query = "SELECT category FROM category_item";
        $result = mysql_query($query) or die(mysql_error());
